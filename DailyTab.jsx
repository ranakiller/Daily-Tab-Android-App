...
async function sGet(k, fb) {
  try { if(window.storage){ const r=await window.storage.get(k); if(r) return JSON.parse(r.value); } } catch(_) {}
  try { const v=localStorage.getItem(k); if(v!==null) return JSON.parse(v); } catch(_) {}
  return fb;
}
async function sSet(k, v) {
  const s = JSON.stringify(v);
  try { if(window.storage) await window.storage.set(k,s); } catch(_) {}
  try { localStorage.setItem(k,s); } catch(_) {}
}

/* ─── design tokens (CSS variables — updated by theme effect) ── */
...
/* ─── tiny reusable components (pure JSX) ───────────────────── */
function Card({ children, style }) {
  return (
    <div style={{ background:CARD, border:`1px solid ${BDR}`, borderRadius:16, padding:16, ...style }}>
      {children}
    </div>
  );
}
  ...
  return (
    <input
      ...
      style={{
        ...
        padding:"11px 13px", color:TXT, fontSize:15, width:"100%",
...
function Lbl({ children }) {
  return (
    <div style={{ fontSize:11, color:MT, letterSpacing:1, textTransform:"uppercase", marginBottom:6, fontWeight:700 }}>
      {children}
    </div>
  );
}
...
function Row({ children, style, onClick }) {
  return (
    <div onClick={onClick} style={{ display:"flex", alignItems:"center", justifyContent:"space-between", ...style }}>
      {children}
    </div>
  );
}
...
function MonNav({ vm, setVm, CM }) {
  return (
    <Row style={{ marginBottom:16 }}>
      <button
        onClick={() => setVm(prevMon(vm))}
        style={{ background:"none", border:`1px solid ${BDR}`, borderRadius:9, padding:"7px 16px", color:TXT, cursor:"pointer", fontSize:18 }}
      >‹</button>
      <div style={{ fontWeight:700, fontSize:16 }}>{mLabel(vm)}</div>
      <button
        onClick={() => setVm(nextMon(vm))}
        disabled={vm >= CM}
        style={{ background:"none", border:`1px solid ${BDR}`, borderRadius:9, padding:"7px 16px", color:vm>=CM?MT:TXT, cursor:vm>=CM?"not-allowed":"pointer", fontSize:18 }}
      >›</button>
    </Row>
  );
}
...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      const s   = await sGet("dt_shops",   []);
      const it  = await sGet("dt_items",   []);
      const p   = await sGet(`dt_p:${CM}`, []);
      const pay = await sGet(`dt_pay:${CM}`,{});
      const cur = await sGet("dt_cur",     "₹");
      const sc  = await sGet("dt_scurrs",  []);
      const th  = await sGet("dt_theme",   "system");
  ...
  useEffect(() => {
    (async () => {
      ...
      const p = await sGet(`dt_p:${calMon}`, []);
      setPurchR(prev => ({ ...prev, [calMon]: p||[] }));
    })();
  }, [calMon]);

  /* ── persist helpers ── */
  const setShops = async v => { setShopsR(v); await sSet("dt_shops", v); };
  const setItems = async v => { setItemsR(v); await sSet("dt_items", v); };
  ...
  const setTheme = async v => { setThemeR(v); await sSet("dt_theme", v); };
    ...
    // add to saved history (deduplicated, newest first)
    setSavedCurrs(prev => {
  ...
  const delShop = async id => {
    await setShops(shops.filter(s => s.id !== id));
    await setItems(items.filter(i => i.shopId !== id));
    if (lShop === id) setLShop("");
  };
  const addItem = async () => {
    if (!niName.trim() || !niShop || !niPrice) return;
    await setItems([...items, { id:uid(), shopId:niShop, name:niName.trim(), price:+niPrice }]);
    setNiName(""); setNiPrice("");
  };
  const delItem  = async id => setItems(items.filter(i => i.id !== id));
  const updPrice = async (id, price) => {
    await setItems(items.map(i => i.id===id ? { ...i, price:+price } : i));
    setEditP(p => { const n={...p}; delete n[id]; return n; });
  };
  ...
  const savePurch = async (date, shopId, itemId, itemName, qty, price, setMsg, onOk) => {
    ...
    const existing = purch[m] !== undefined ? purch[m] : await sGet(`dt_p:${m}`, []);
    const entry = {
      id:uid(), date, shopId, itemId:itemId||null,
      itemName:itemName.trim(), qty:+qty||1, price:+price,
      total:(+qty||1)*(+price),
    };
  ...
  const logPurch = async () => {
    const name = lItem ? (items.find(i=>i.id===lItem)?.name||lName) : lName.trim();
    await savePurch(lDate, lShop, lItem, name, lQty, lPrice, setLMsg, () => {
      setLItem(""); setLName(""); setLQty("1"); setLPrice("");
    });
  };

  const modalSave = async () => {
    const name = mItem ? (items.find(i=>i.id===mItem)?.name||mName) : mName.trim();
    await savePurch(modal.date, mShop, mItem, name, mQty, mPrice, setMMsg, () => {
      setMItem(""); setMName(""); setMQty("1"); setMPrice("");
    });
  };

  const delPurch  = async (m, id) => setPM(m, (purch[m]||[]).filter(p => p.id!==id));
  const markPaid  = async (m, sid, amt) => {
    const cur = pays[m]||{};
    await setPyM(m, { ...cur, [sid]:(+(cur[sid]||0))+(+amt) });
    setSummPay(p => ({ ...p, [sid]:"" }));
  };
  ...
  try {
    ...
    const _smap = {};
    _mp.forEach(p => {
      const _n = (p.itemName || "Unknown").trim();
      const _k = _n.toLowerCase();
      if (!_smap[_k]) _smap[_k] = { name: _n, shopId: p.shopId || "", dates: new Set() };
      if (p.date) _smap[_k].dates.add(p.date);
    });
    calStats = {
      items: Object.values(_smap).sort((a, b) => b.dates.size - a.dates.size),
      daysElapsed: _de,
      daysInMon: _dim,
    };
  } catch (_e) { /* keep default empty stats */ }
  ...
  return (
    <div style={{ background:BG, minHeight:"100vh", fontFamily:"-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif", color:TXT, paddingBottom:72, maxWidth:480, width:"100%", margin:"0 auto", boxSizing:"border-box", overflowX:"hidden" }}>
      ...
      {/* ── header ── */}
      <div style={{ background:CARD, borderBottom:`1px solid ${BDR}`, padding:"14px 18px", position:"sticky", top:0, zIndex:10 }}>
        <Row>
          <div>
      ...
      {tab==="home" && (
        <div style={{ padding:16 }}>
          ...
          {shops.length === 0
            ? (
              <Card style={{ textAlign:"center", color:MT, fontSize:13 }}>
                No shops yet.{" "}
                <span style={{ color:IND, cursor:"pointer" }} onClick={() => setTab("settings")}>Add in Settings →</span>
              </Card>
            )
            : shops.map(sh => {
      ...
      {tab==="log" && (
        <div style={{ padding:16 }}>
          <Card>
            ...
            {shops.length===0 && (
              <div style={{ textAlign:"center", color:MT, fontSize:13, marginBottom:12 }}>
                <span style={{ color:IND, cursor:"pointer" }} onClick={() => setTab("settings")}>Add a shop in Settings first →</span>
              </div>
            )}
            {lShop && (
              <div>
      ...
      {tab === "calendar" && (
        <div style={{ padding: 16 }}>
          ...
          <div style={{ display:"flex", gap:8, flexWrap:"wrap", marginBottom:14 }}>
            <button
              onClick={() => setCalFilt("")}
              style={{ background: calFilt==="" ? IND : CARD2, color: calFilt==="" ? "#fff" : MT, border:`1px solid ${calFilt===""?IND:BDR}`, borderRadius:20, padding:"5px 14px", fontSize:12, fontWeight:600, cursor:"pointer" }}
            >All Shops</button>
            {shops.map(sh => (
              <button
                key={sh.id}
                onClick={() => setCalFilt(calFilt===sh.id ? "" : sh.id)}
                style={{ background: calFilt===sh.id ? sh.color : CARD2, color: calFilt===sh.id ? "#fff" : MT, border:`1px solid ${calFilt===sh.id?sh.color:BDR}`, borderRadius:20, padding:"5px 14px", fontSize:12, fontWeight:600, cursor:"pointer", display:"flex", alignItems:"center", gap:5 }}
              >
                <div style={{ width:7, height:7, borderRadius:4, background: calFilt===sh.id ? "#fff" : sh.color }} />
                {sh.name}
              </button>
            ))}
          </div>
          ...
          <Card style={{ padding:10, overflowX:"hidden" }}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(7,1fr)", marginBottom:4 }}>
              {DOW.map(d => (
                <div key={d} style={{ textAlign:"center", fontSize:10, fontWeight:700, color:MT, padding:"3px 0" }}>{d}</div>
              ))}
            </div>
            {/* date cells */}
            <div style={{ display:"grid", gridTemplateColumns:"repeat(7,1fr)", gap:2 }}>
              {calDays(calMon).map((d, i) => {
                ...
                const ds       = padDate(calMon, d);
                const shopDots = getDateShops(ds);
                const isToday  = ds === today();
                const hasPurch = shopDots.length > 0;
                return (
                  <div
                    key={d}
                    onClick={() => openModal(ds)}
                    style={{
                      borderRadius:8, padding:"4px 1px", textAlign:"center", cursor:"pointer",
                      background: isToday ? `${IND}22` : hasPurch ? CARD2 : "transparent",
                      border: isToday ? `1.5px solid ${IND}` : `1px solid ${hasPurch?BDR:"transparent"}`,
                  ...
                  >
                    ...
                    <div style={{ display:"flex", gap:1, flexWrap:"wrap", justifyContent:"center", minHeight:9 }}>
                      {shopDots.slice(0, 3).map(sid => {
                        const sh = byShop(sid);
                        return <div key={sid} style={{ width:5, height:5, borderRadius:3, background:sh?sh.color:MT, flexShrink:0 }} />;
                      })}
                    </div>
                  </div>
                );
          ...
          {shops.length > 0 && (
            <div style={{ display:"flex", gap:12, flexWrap:"wrap", marginTop:10, padding:"0 2px" }}>
              {shops.map(sh => (
                <div key={sh.id} style={{ display:"flex", alignItems:"center", gap:5, fontSize:12, color:MT }}>
                  <div style={{ width:8, height:8, borderRadius:4, background:sh.color }} />
                  {sh.name}
                </div>
              ))}
            </div>
          ...
          <Card style={{ marginTop:14 }}>
            <Row style={{ marginBottom: calStats.items.length > 0 ? 14 : 0 }}>
              ...
              <div style={{ fontSize:12, color:MT }}>
                {calMon === CM
                  ? `${calStats.daysElapsed} of ${calStats.daysInMon} days`
                  : `${calStats.daysInMon} days`}
              </div>
            </Row>

            {calStats.items.length === 0 ? (
              <div style={{ textAlign:"center", padding:"18px 0", color:MT, fontSize:13 }}>
                No entries yet for {mLabel(calMon)}.{"\n"}Tap any date above to log a purchase.
              </div>
            ) : (
              calStats.items.map(({ name, shopId, dates }) => {
                const sh     = byShop(shopId);
                const bought = dates.size;
                const missed = Math.max(0, calStats.daysElapsed - bought);
                const pct    = calStats.daysElapsed > 0 ? Math.min(100, (bought / calStats.daysElapsed) * 100) : 0;
                return (
                  <div key={name} style={{ marginBottom:18 }}>
                    ...
                    <Row style={{ marginBottom:8 }}>
                      <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                        ...
                        {sh && (
                          <span style={{ fontSize:11, color:MT, background:CARD2, padding:"1px 8px", borderRadius:10 }}>{sh.name}</span>
                        )}
                      </div>
                      <span style={{ fontSize:14, fontWeight:800, color:AMB }}>
                        {bought}<span style={{ fontSize:11, fontWeight:400, color:MT }}>/{calStats.daysElapsed}d</span>
                      </span>
                    </Row>
            ...
            {calStats.items.length > 0 && (
              <div style={{ fontSize:11, color:MT, textAlign:"center", paddingTop:8, borderTop:`1px solid ${BDR}` }}>
                {calFilt ? `Shop: ${byShop(calFilt)?.name || ""}` : "All shops"} · tap any date to add/view entries
              </div>
            )}
          </Card>
        ...
        <div style={{ padding:16 }}>
          ...
          {shops.length === 0
            ...
            : shops.map(sh => {
                ...
                const isOpen = summExp === sh.id;
                const shP = vmP.filter(p => p.shopId===sh.id);
                const shBD = {};
                ...
                return (
                  <Card key={sh.id} style={{ marginBottom:14 }}>
                    ...
                    {tot > 0 && (
                      <div style={{ height:6, background:BDR, borderRadius:6, marginBottom:14, overflow:"hidden" }}>
                        <div style={{ height:"100%", width:`${Math.min(100,(paid/tot)*100)}%`, background:GRN, borderRadius:6 }} />
                      </div>
                    )}
                    {owed > 0 && (
                      ...
                      <div>
                        ...
                        {paid > 0 && (
                          <div>
                            <Divider />
                            <Row>
                              <span style={{ fontSize:13, color:GRN }}>Paid so far: {fmt(paid)}</span>
                              <Btn sm outline danger onClick={() => resetPaid(vm,sh.id)}>Reset</Btn>
                            </Row>
                          </div>
                        )}
                      </div>
          ...
          {shops.length > 0 && (
            <Card style={{ background:CARD2 }}>
              <Row>
                <span style={{ color:MT, fontSize:14 }}>Month Grand Total</span>
                <span style={{ fontSize:21, fontWeight:800 }}>{fmt(vmP.reduce((a,p)=>a+p.total,0))}</span>
              </Row>
              <Row style={{ marginTop:6 }}>
                <span style={{ color:MT, fontSize:13 }}>Still Owed</span>
                <span style={{ fontSize:17, fontWeight:700, color:RED }}>{fmt(shops.reduce((a,s)=>a+sOwed(s.id,vm),0))}</span>
              </Row>
            </Card>
          )}
        </div>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:10 }}>
              {[
                ...
                <button
                  ...
                  style={{
                    background: theme===opt.id ? IND : CARD2,
                    color:      theme===opt.id ? "#fff" : TXT,
                    border:     `1.5px solid ${theme===opt.id ? IND : BDR}`,
          ...
          <Card style={{ marginBottom:16 }}>
            ...
            <div style={{ display:"flex", gap:8, marginBottom:savedCurrs.length > 0 ? 14 : 0 }}>
              <Inp
                value={currInp}
                onChange={setCurrInp}
                placeholder="Type any symbol e.g. ₹ $ € £ AED"
              />
              ...
              <div>
                ...
                <div style={{ display:"flex", flexWrap:"wrap", gap:8 }}>
                  {savedCurrs.map(c => (
                    <button
                      key={c}
                      onClick={() => { setCurrInp(c); applyCurrency(c); }}
                      style={{
                        background: currency===c ? IND : CARD2,
                        color: currency===c ? "#fff" : TXT,
                        border: `1.5px solid ${currency===c ? IND : BDR}`,
                    ...
                    >
                      {c}
                      {currency===c && <span style={{ fontSize:10, opacity:.8 }}>✓</span>}
                    </button>
                  ))}
                </div>
              </div>
            )}
            <div style={{ marginTop:12, fontSize:13, color:MT }}>
              Active: <span style={{ color:AMB, fontWeight:800, fontSize:16 }}>{currency}</span>
            </div>
          </Card>
        ...
        {NAV.map(t => (
          <button
            ...
            style={{
              ...
              padding:"8px 0 6px", display:"flex", flexDirection:"column",
              alignItems:"center", gap:2,
              color: tab===t.id ? IND : MT,
              borderTop: `2px solid ${tab===t.id?IND:"transparent"}`,
            }}
          >
